...
Email can be delivered through Resend, Mailgun, or SMTP. When more than one is configured, priority is `SMTP_HOST` → `MAILGUN_API_KEY` → `RESEND_API_KEY`.
...
1. Verify a sending domain in [Mailgun](https://www.mailgun.com/) and copy its private API key.
2. Set:
...
3. EU-region domains must also set `MAILGUN_API_BASE_URL=https://api.eu.mailgun.net` — Mailgun rejects requests for an EU domain sent to the default US host (and vice versa). Check which region your domain is on in the Mailgun dashboard.
...
Mailgun also offers SMTP credentials as an alternative to `MAILGUN_API_KEY` — useful if you'd rather not provision a separate HTTP API key. They are managed separately from the API key in the Mailgun dashboard (Sending → your domain → SMTP credentials).
...
The startup log states whether the current mode is `Resend API`, `Mailgun API`, `SMTP relay`, or `DEV mode`.
