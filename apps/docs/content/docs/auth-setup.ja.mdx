...
メールは Resend、Mailgun、SMTP のいずれかで送信できます。複数設定した場合の優先順位は `SMTP_HOST` → `MAILGUN_API_KEY` → `RESEND_API_KEY` です。
...
1. [Mailgun](https://www.mailgun.com/) で送信ドメインを検証し、private API key をコピーします。
2. 次を設定します:
...
3. EU リージョンのドメインでは `MAILGUN_API_BASE_URL=https://api.eu.mailgun.net` も設定してください。デフォルトの US ホストに EU ドメインのリクエストを送る（またはその逆の）と Mailgun に拒否されます。ドメインのリージョンは Mailgun ダッシュボードで確認できます。
4. API サービスを再起動します。
...
Mailgun は `MAILGUN_API_KEY` の代わりに SMTP 認証情報も提供しています。別途 HTTP API key を発行したくない場合に使えます。SMTP 認証情報は Mailgun ダッシュボード（Sending → 対象ドメイン → SMTP credentials）で API key とは別に管理されています。
...
起動ログには、現在使っているのが `Resend API`、`Mailgun API`、`SMTP relay`、`DEV mode` のどれかが表示されます。
