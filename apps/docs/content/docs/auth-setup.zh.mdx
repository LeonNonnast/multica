...
邮件可以通过 Resend、Mailgun 或 SMTP 发送。同时配置多个时，优先级为 `SMTP_HOST` → `MAILGUN_API_KEY` → `RESEND_API_KEY`。
...
1. 在 [Mailgun](https://www.mailgun.com/) 验证发信域名，并复制其私有 API key。
2. 设置：
...
3. EU 区域的域名还需设置 `MAILGUN_API_BASE_URL=https://api.eu.mailgun.net`——发往默认 US 主机的 EU 域名请求（反之亦然）会被 Mailgun 拒绝。可在 Mailgun 控制台查看域名所在区域。
4. 重启 API 服务。
...
Mailgun 也提供 SMTP 凭据作为 `MAILGUN_API_KEY` 之外的另一种方式——如果不想单独申请 HTTP API key，可以使用它。SMTP 凭据与 API key 在 Mailgun 控制台中是分开管理的（Sending → 对应域名 → SMTP credentials）。
...
启动日志会说明当前使用的是 `Resend API`、`Mailgun API`、`SMTP relay` 还是 `DEV mode`。
