...
func main() {
	...
	if os.Getenv("RESEND_API_KEY") == "" && strings.TrimSpace(os.Getenv("SMTP_HOST")) == "" && strings.TrimSpace(os.Getenv("MAILGUN_API_KEY")) == "" {
		slog.Warn("no email backend configured (RESEND_API_KEY, SMTP_HOST, and MAILGUN_API_KEY all empty) — verification codes will be printed to the log instead of emailed.")
	}
